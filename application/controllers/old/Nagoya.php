...
// 			
		...
		foreach($data as $dt){
			...
			if($check==2){
				...
				$this->nagoyadb->where('id', $dt['id']);
				$this->nagoyadb->update('datacetak', $data);
			}
		}
		redirect('?msg=success&&com=nagoya DATA CETAK&&count='.$count);
	}
	...
	function upload_pengeluaran(){
		...
		redirect('?msg=success&&com=nagoya PENGELUARAN&&count='.$count);
	}
	...
	function upload_kas(){
		...
		redirect('?msg=success&&com=nagoya KAS HARIAN&&count='.$count);
	}
